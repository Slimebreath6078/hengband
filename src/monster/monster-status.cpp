...
int mon_damage_mod(PlayerType *player_ptr, MonsterEntity *m_ptr, int dam, bool is_psy_spear)
{
    ...
    if (race_info.special_flags.has(MonsterSpecialType::LAFFEY_II)) {
        race_info.r_special_flags.set(MonsterSpecialType::LAFFEY_II);
        if (dam > m_ptr->hp / 10) {
            dam = std::max(m_ptr->hp / 10, m_ptr->maxhp * 9 / 1000);
